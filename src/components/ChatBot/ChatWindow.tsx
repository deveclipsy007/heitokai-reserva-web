...
const ChatWindow = ({
  ...
  const [messages, setMessages] = useState<Message[]>([{
    role: 'assistant',
    content: 'Olá! 😊 Que prazer ter você por aqui! Sou o assistente do Reserva Rio Uru. Como posso ajudá-lo hoje?'
  }]);
  ...
  return <div className="bg-white rounded-lg shadow-xl w-80 sm:w-96 h-[500px] flex flex-col overflow-hidden border border-gray-200">
      <div className="p-4 bg-heitokai-green text-white font-medium flex justify-between items-center bg-green-900">
      ...
      <ScrollArea className="flex-1 p-4">
        <div className="space-y-4">
          ...
          duration: 0.3
              ...
              <div className={`max-w-[80%] p-3 rounded-lg ${msg.role === 'user' ? 'bg-heitokai-green text-white rounded-tr-none' : 'bg-gray-100 text-gray-800 rounded-tl-none'}`}>
                <div className="flex items-start gap-2">
                  {msg.role === 'assistant' && <Bot className="h-5 w-5 mt-1 flex-shrink-0" />}
                  <div className="text-sm">{msg.content}</div>
                  {msg.role === 'user' && <User className="h-5 w-5 mt-1 flex-shrink-0" />}
                </div>
              </div>
            </motion.div>)}
          {(isLoading || isTypingSecondMessage) && <div className="flex justify-start">
      ...
      <form onSubmit={handleSubmit} className="p-3 border-t border-gray-200 flex gap-2">
        <Input value={input} onChange={e => setInput(e.target.value)} placeholder="Digite sua mensagem..." className="flex-1" disabled={isLoading || isTypingSecondMessage} />
        <Button type="submit" size="icon" disabled={isLoading || isTypingSecondMessage || !input.trim()} className="bg-heitokai-green hover:bg-heitokai-dark text-slate-50 bg-green-950 hover:bg-green-800">
